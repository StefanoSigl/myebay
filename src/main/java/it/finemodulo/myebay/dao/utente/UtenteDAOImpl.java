...
public class UtenteDAOImpl implements UtenteDAO {
	...
	public List<Utente> findByExample(Utente example) {
		...
		if (example.getRuoli() != null) {
			...
			whereClauses.add("r.id in(:ruoli)");
			paramaterMap.put("ruoli", idRuoli);
	...
	public Optional<Utente> findOneEager(long parseLong) {
		return entityManager.createQuery("from Utente u left join fetch u.ruoli f where u.id=:idUtente", Utente.class)
				.setParameter("idUtente", parseLong).getResultList().stream().findFirst();
	}
