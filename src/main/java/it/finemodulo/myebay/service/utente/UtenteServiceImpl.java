...
public class UtenteServiceImpl implements UtenteService {
	...
	public Utente caricaSingoloElementoEager(long parseLong) {
		...
		try {
			...
			// eseguo quello che realmente devo fare
			return utenteDAO.findOneEager(parseLong).orElse(null);
	...
	public void disassociaRuoli(Utente loadedUtente) {
		...
		try {
			...
			loadedUtente = entityManager.merge(loadedUtente);
			Set<Ruolo> ruoliToRemove = new HashSet<>();
			for (Ruolo ruoloItem : loadedUtente.getRuoli()) {
				ruoloItem = entityManager.merge(ruoloItem);
				ruoliToRemove.add(ruoloItem);
			}
			loadedUtente.getRuoli().removeAll(ruoliToRemove);
	...
	public void aggiornaUtenteConRuoli(Utente utenteInstance, List<Ruolo> listRuoli, String[] roleCheck) {
		...
		try {
			...
			for (Ruolo ruoloItem : listRuoli) {
				utenteInstance.getRuoli().remove(entityManager.merge(ruoloItem));
			}
			for (String ruoloItemId : roleCheck) {
				utenteInstance.getRuoli().add(new Ruolo(Long.parseLong(ruoloItemId)));
			}
